...
class _DesignerShellMixin:
    ...
    def mode(self: Any) -> str:
        """Return current Acherion view mode."""
    ...
    def has_user_logic(self: Any) -> bool:
        ...
        if any(not self._is_system_node(n) for n in self._graph.nodes):
            return True
        return any(
            self._is_system_sink_node(n) and str(n.params.get("source") or "").strip()
    ...
    def _clear_render_cache_storage(self: Any) -> None:
        ...
        self._node_bounds_cache_revision = -1
        self._node_bounds_cache = {}
        self._function_box_bounds_cache = {}
        self._node_lookup_cache_revision = -1
        self._node_lookup_cache = {}

    def _invalidate_graph_render_caches(self: Any) -> None:
        self._graph_cache_revision = int(getattr(self, "_graph_cache_revision", 0)) + 1
        self._clear_render_cache_storage()

    def set_graph_from_dict(self: Any, data: dict[str, Any] | None) -> None:
        """Replace graph state from persisted builder_state data."""
        self._invalidate_graph_render_caches()
    ...
    def preview_bindings(self: Any) -> dict[str, dict[str, Any]]:
        ...
        return {scope: dict(values) for scope, values in self._preview_bindings.items()}
    ...
    def generated_user_code(self: Any) -> str:
        ...
        raise RuntimeError("AcherionDesigner requires a host to generate user code.")
    ...
    def build(self: Any) -> None:
        ...
        with (
            ui.element("div")
            .classes("ach-workbench oe-surface rounded-lg w-full min-w-0")
            ...
            with ui.element("div").classes("ach-workbench-body"):
                with ui.element("div").classes("ach-workbench-main") as graph_host:
                    ...
                    with (
                        ui.element("div")
                        .classes("ach-shell")
                        .props(f"tabindex=0 id={self._viewport_dom_id}") as viewport
                    ):
    ...
    def refresh(self: Any) -> None:
        ...
        with self._canvas_el:
            ...
            if not self._graph.nodes:
                ui.label("No nodes yet. Add components from the left sidebar.").classes(
                    "ach-empty text-sm"
                ).style(
    ...
    def _client_interaction_state_payload(self: Any) -> dict[str, Any]:
        ...
        if pending_source_id:
            ...
            for node in self._graph.nodes:
                for input_index, pin in enumerate(self._input_pin_specs(node)):
                    ...
                    if _catalog_types.types_compatible(
                        pending_type,
                        target_type,
                    ):
                        continue
        ...
        return {
            ...
            "selected_connection_id": self._selected_connection_id or "",
            "pending_source_id": pending_source_id,
    ...
    def _sync_client_interaction_state(self: Any) -> None:
        canvas_id = getattr(self, "_canvas_dom_id", "")
        if not canvas_id:
            return
        payload = json.dumps(self._client_interaction_state_payload())
        self._run_client_javascript(
            f"(function(){{"
            f"const canvas=document.getElementById({canvas_id!r});"
            f"const h=window.__oeAcherion;"
            f"if(!canvas||!h)return;"
            f"h.applyInteractionState(canvas,{payload});"
            f"}})()"
        )

    def _sync_client_selection_state(self: Any) -> None:
        self._sync_client_interaction_state()

    def _apply_client_connection_patch(
        self: Any,
        patch: dict[str, Any],
    ) -> None:
        canvas_id = getattr(self, "_canvas_dom_id", "")
        if not canvas_id:
            return
        payload = dict(patch)
        payload["state"] = self._client_interaction_state_payload()
        ...
        self._run_client_javascript(
            ...
            f"const h=window.__oeAcherion;"
            f"if(!canvas||!h)return;"
            f"h.applyConnectionPatch(canvas,{patch_json});"
            f"}})()"
        )
    ...
    def _render_toolbar_menu(
        ...
        with (
            ui.button(label)
            .props(f"flat dense no-caps color=white id={button_dom_id}")
            .classes("ach-menu-button")
        ):
            with (
                ui.menu()
                .props(
        ...
        with ui.element("div").classes("ach-menubar"):
            with ui.element("div").classes("ach-menubar-left"):
                with ui.row().classes("ach-menubar-menus"):
                    ...
                    self._mode_toggle_btn = (
                        ...
                        .classes("ach-mode-button")
                    )
                    ui.button(
                        "Compile",
                        icon="play_arrow",
                        on_click=self._apply_to_code,
                    ).props("unelevated dense no-caps").classes("ach-compile-button")
                    ui.button(
                        "Validate",
                        icon="check_circle",
            ...
            with ui.element("div").classes("ach-menubar-right"):
                with ui.element("div").classes("ach-statusbar"):
                    ...
                    self._hint_label = ui.label("").classes("ach-statusbar-text")
                    self._update_hint()
    ...
    def _render_help_dialog_body(self: Any) -> None:
        ...
        with self._help_topics_container:
            if not topics:
                ui.label("No help topics found.").classes("ach-preferences-empty")
            for topic in topics:
                ...
                with ui.element("div").classes("ach-preferences-dialog-shell"):
                    ...
                    with body_container:
                        with ui.element("div").classes("ach-preferences-body"):
                            with ui.element("div").classes("ach-preferences-sidebar"):
                                with ui.element("div").classes(
                                    ...
                                    search_input = (
                                        ...
                                        .props(f"id={self._frame_dom_id}-help-search")
                                    )
                            ...
                            with ui.element("div").classes("ach-preferences-main"):
                                self._help_content_container = ui.element(
                                    "div"
                                ).classes("ach-preferences-content ach-help-content")
                    self._render_help_dialog_body()
                    with ui.element("div").classes("ach-preferences-footer"):
                        ui.button(
                            ...
                            "flat no-caps"
                        ).classes("ach-preferences-close-text")
        dialog.open()
    ...
    def _validate_current(self: Any) -> None:
        if self._on_validate is None:
            self._notify_ui(
                "Validation is not available for this Acherion instance.",
                type="warning",
            )
            return
        self._on_validate()
