...
class _RenderPinsMixin:
    ...
    def _render_input_literal_editor(
        ...
        *,
        ...
        text_field.on('update:model-value', _stage_literal_text)
        text_field.on(
            'blur',
    ...
    def _body_input_pins(
        ...
        return [
            ...
            if not _is_exec_pin(pin) or top_exec is None or index != top_exec[0]
        ]
    ...
    def _body_pin_rows(
        ...
        for output_position, (_pin_index, pin) in enumerate(body_outputs):
            ...
            if match_key is None:
                continue
            output_positions_by_key.setdefault(match_key, []).append(output_position)

        used_output_positions: set[int] = set()
        rows: list[dict[str, tuple[int, dict[str, str]] | None]] = []
    ...
    def _body_pin_row_count(
        ...
        if node.kind == "for_each":
            data_outputs = [
                pin
                for _index, pin in self._body_output_pins(node)
                if not _is_exec_pin(pin)
            ]
            return 1 + len(data_outputs)
        if node.kind == "sequencer":
            exec_outputs = [
                pin for _index, pin in self._body_output_pins(node) if _is_exec_pin(pin)
            ]
    ...
    def _body_pin_row_index(
        ...
        *,
        ...
        if node.kind == "else_if_branch":
            ...
            condition_indexes = [index for index, _pin in self._body_input_pins(node)]
        ...
        if node.kind == "for_each":
            if direction == "out":
                ...
                if pin_index < len(output_specs):
                    ...
                    data_indexes = [
                        index
                        for index, pin in enumerate(output_specs)
                        if not _is_exec_pin(pin)
                    ]
                    if pin_index in data_indexes:
                        return data_indexes.index(pin_index) + 1
                return 0
            input_indexes = [index for index, _pin in self._body_input_pins(node)]
    ...
    def _render_top_exec_row(
        ...
        if exec_input is None and exec_output is None and not entry_source_id:
            return
        with ui.row().classes("ach-wire-row ach-exec-row"):
            with ui.row().classes("items-center gap-2"):
                if exec_input is not None:
                    ...
                    ui.element("div").classes(f"{btn_cls} ach-pin-anchor").props(
